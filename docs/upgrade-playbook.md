...
10. Run the opt-in model evaluations with an explicitly configured provider credential: `RUN_MODEL_EVALS=1 node scripts/run-evals.mjs --json > eval-evidence.json` (`node` directly, not `pnpm eval` — see `evals/README.md`). Record correct target, successful tool result, persisted state, human approval behavior, and member denial. If no credential is available, report this release evidence as pending; a skipped run is not model validation.
